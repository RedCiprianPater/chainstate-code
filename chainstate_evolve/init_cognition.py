...
Auto-discovers where ASI-Evolve lives (env var -> sibling dir -> parent) and
adds it to sys.path, so this script can be run from anywhere:

    python chainstate_evolve/init_cognition.py
...
def _find_asi_evolve():
    """Return the absolute path to ASI-Evolve, or None."""
    env_root = os.environ.get("ASI_EVOLVE_ROOT")
    if env_root:
        p = os.path.abspath(env_root)
        if os.path.isdir(p):
            return p
    for candidate in [
        os.path.join(PROJECT_ROOT, "ASI-Evolve"),
        os.path.join(os.path.dirname(PROJECT_ROOT), "ASI-Evolve"),
    ]:
...
asi_root = _find_asi_evolve()
if asi_root:
    sys.path.insert(0, asi_root)

try:
    from cognition.store import CognitionStore
except ImportError as e:
    print(f"x Cannot import cognition.store -- {e}", file=sys.stderr)
    print(f"  Detected ASI_EVOLVE_ROOT: {asi_root or '(not found)'}", file=sys.stderr)
    ...
    print(f"  Fix: clone ASI-Evolve at project root, or set ASI_EVOLVE_ROOT to an absolute path.",
          file=sys.stderr)
    sys.exit(1)
...
store.add([
    {"title": "CHAINSTATE Subspace Layout", "content": (
    ...
    {"title": "Log-Pool Convergence Kernel", "content": (
        ...
        "CODE whitepaper.")},
    {"title": "Same-Subspace Density Prior", "content": (
    ...
    {"title": "Substrate-Subspace Affinity", "content": (
    ...
    {"title": "KV Cache Behavior", "content": (
    ...
    {"title": "Swarm Size vs Depth Trade-off", "content": (
    ...
    {"title": "NWO-ASM Opcode Constraints", "content": (
    ...
    {"title": "Symbol Pools by Subspace", "content": (
    ...
    {"title": "Objective Restated", "content": (
    ...
    {"title": "Ownership and Governance", "content": (
...
print(f"OK Seeded {STORAGE_DIR} with 10 CHAINSTATE cognition items")
print(f"   ASI-Evolve root: {asi_root}")
